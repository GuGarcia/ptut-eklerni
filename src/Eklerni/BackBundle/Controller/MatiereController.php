...
class MatiereController extends Controller
{
    public function ajouterAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $this->get("session")->getFlashBag()->add("notice", $this->get("translator")->trans("matiere.add.success"));
    ...
    public function modifierAction(Request $request, $idMatiere)
    {
        ...
        if ($form->isValid()) {
            ...
            $this->get("session")->getFlashBag()->add("notice", $this->get("translator")->trans("matiere.modify.success"));
            return $this->redirect($this->generateUrl('eklerni_back_direction'));
        } else {
            return $this->render(
            ...
            if (!$matiere) {
                return new Response(
                    json_encode(
                        array(
                            "success" => false,
                            'message' => $this->get("translator")->trans("matiere.notfound")
                        )
                    )
                );
            }
